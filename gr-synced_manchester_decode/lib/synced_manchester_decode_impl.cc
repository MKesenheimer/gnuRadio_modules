...
#include <volk/volk.h>
#include "synced_manchester_decode_impl.h"

namespace gr {
  namespace synced_manchester_decode {

    template <class T>
    typename synced_manchester_decode<T>::sptr synced_manchester_decode<T>::make(size_t samples_per_symbol, size_t n_sync_symbols, int bit_mode, int endianess) {
      return gnuradio::make_block_sptr<synced_manchester_decode_impl<T>>(
     ...
     */
    template <class T>
    synced_manchester_decode_impl<T>::synced_manchester_decode_impl(size_t samples_per_symbol, size_t nsync_symbols, int bit_mode, int endianess)
      : gr::block("synced_manchester_decode",
    ...
    template <class T>
    void synced_manchester_decode_impl<T>::forecast(int noutput_items, gr_vector_int &ninput_items_required) {
#ifdef DEBUG
    ...
    template <class T>
    int synced_manchester_decode_impl<T>::general_work (int noutput_items,
      ...
      for (size_t i = 0; i <= nsamples; ++i) {
        ...
        if (start) {
          ...
          if (sample != last_sample || i == nsamples) {
            ...
            // count the number of equal samples and compare the symbols when there was a change
    ...
    template class synced_manchester_decode<std::uint8_t>;
    template class synced_manchester_decode<std::int16_t>;
    template class synced_manchester_decode<std::int32_t>;
    template class synced_manchester_decode<float>;
  } /* namespace synced_manchester_decode */
} /* namespace gr */
